...
public class UnfreezeCommand implements CommandExecutor, TabCompleter {
    ...
    public UnfreezeCommand(KoalaManaging plugin) { this.plugin = plugin; }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd,
        ...
        if (args.length < 1) {
            sender.sendMessage(plugin.prefix() + "\u00a7cUsage: /unfreeze <player>"); return true;
        }
        ...
        if (!plugin.getFreezeManager().isFrozen(target.getUniqueId())) {
            sender.sendMessage(plugin.prefix() + "\u00a7c" + target.getName() + " is not frozen."); return true;
        }
        plugin.getFreezeManager().unfreeze(target.getUniqueId());
        target.removePotionEffect(PotionEffectType.BLINDNESS);
        target.sendMessage(plugin.prefix() + "\u00a7aYou have been \u00a7lunfrozen\u00a7a by " + sender.getName() + ".");
        sender.sendMessage(plugin.prefix() + "\u00a7a" + target.getName() + " \u00a77has been unfrozen.");
        return true;
    }
